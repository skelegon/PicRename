...
public class Main {
    ...
    public static void main(String[] args) throws IOException  {
        ...
        File[] images = getPictures(curDir);
        if (images.length==0) System.exit(0);

        // Delete duplicates
        deleteDuplicatePictures(images);

        // List the images in current directory after removing the duplicate files
        images = getPictures(curDir);
    ...
    private static void deleteDuplicatePictures(File[] images) {
        for (int x = 0; x < images.length; x++) {
            for (int y = x + 1; y < images.length; y++) {
                if (CompareFiles(images[x], images[y])) images[x].delete();
            }
        }
    }
